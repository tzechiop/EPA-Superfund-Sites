...
fp <- "C:/Users/thasegawa/Documents/68 NYC DEP Papers/05 Data/Newtown Creek/TPAH GW Concentration"
aoi <- readOGR(paste(file.path(fp)), "NCG_NYCDEP_GW_locs_Poly_Clipped",
               stringsAsFactors = F)
aoi <- spTransform(aoi, CRS("+init=EPSG:26918"))

# UltraSeep locations
fp <- "C:/Users/thasegawa/Documents/68 NYC DEP Papers/05 Data/Newtown Creek/TPAH GW Concentration"
us <- readOGR(paste(file.path(fp)), "NCG_NYCDEP_GW_samplinglocations",
              stringsAsFactors = F)
us <- spTransform(us, CRS("+init=EPSG:26918"))

#Update us shapefile with true average vert seepage rate
m <- match(us$Location, aoi$Location)
us$TPAH_ug_L <- aoi$TPAH_ug_L[m]
# us$ModVqCMD <- aoi$ModVqCMD[m]
# us$TPAH_ug_L[which(us$Location == "NTC21")] <- 0
...
nc_grd_over$USeepID <- us$Location
...
points(us_str_points_spdf[which(us_str_points_spdf$Location == "NTC"), ], 
       col = "black", pch = 20)
...
idw_sg <- idw(formula = TPAH_ug_L~1, locations = data,  newdata = sg) #idp = 2
...
plot(us_str_points_spdfSUB$TPAH_ug_L, extract(sgRaster, us_str_points_spdfSUB),
     ylab = "IDW Pct Fines (%)", xlab = "Measured Pct Fines (%)",
     pch = 20, cex = 1.5, ylim = c(0, 100), xlim = c(0, 100))
lines(c(-1000, 1000), c(-1000, 1000), lty = 2, lwd = 1)
...
#   tmp <- data[-i, ]
#   # Inverse distance weighting
#   idw_sg0 <- idw0(formula = TPAH_ug_L~1, data = data[-i, ],  newdata = data[i, ])
#   LOOCV_Pred[i] <- idw_sg$var1.pred
#   LOOCV_Pred0[i] <- idw_sg0
# }
# plot(data@data$TPAH_ug_L, (LOOCV_Pred0),
#      ylab = "Pct Fines LOOCV IDW prediction (%)", xlab = "Measured Pct Fines (%)",
#      pch = 20, cex = 1.5, xlim = c(0, 100), ylim = c(0,100))
# lines(c(-1000, 1000), c(-1000, 1000), lty = 2, lwd = 1)
...
#          "NTConly_hydrogrid_IDWNug", driver="ESRI Shapefile")

#####---------------------------------------------------------------------------
#####---------------------------------------------------------------------------
# Export x,y coordinates for straightened points and modeled and observed seepage
m <- match(us_str_points_spdfSUB$Location, aoi$Location)
us_str_points_spdfSUB$TPAH_ug_L <- aoi$TPAH_ug_L[m]
# us_str_points_spdfSUB$ModVqCMD <- aoi$ModVqCMD[m]
# us_str_points_spdfSUB$TPAH_ug_L[which(us_str_points_spdfSUB$Location == "NTC21")] <- 0.01

sub_out <- us_str_points_spdfSUB@data[c("Location", "X", "Y", "TPAH_ug_L")]
...
# Specify para
cutoff = 50 #max lag distance for variogram
width = cutoff/17 #lag width
alpha = 90
tolhor = 25

# calculate sample variogram
vgm = variogram(log(TPAH_ug_L) ~ 1, data = dat,
                cutoff = cutoff, width=width, alpha = c(alpha),
                tol.hor = tolhor)

# Calculate model variogram and variogram line
null.vgm <- vgm("Sph") #vgm(var(data$TPAH_ug_L), "Sph") , nugget=0.2) # initial parameters #range = sqrt(areaSpatialGrid(sg))/4
#null.vgm <- vgm(var(data$TPAH_ug_L), "Sph", range = sqrt(areaSpatialGrid(sg))/4) #, nugget=0.2) # initial parameters #range = sqrt(areaSpatialGrid(sg))/4
vgm.fit <- fit.variogram(vgm, model=null.vgm, fit.sills = T, fit.ranges = T)
variogram = autofitVariogram(log(TPAH_ug_L) ~ 1, input_data = dat, model = "Sph")
...
mtext(side = 2, text = "Semi-variance", line = 2)
lines(varioLine$dist, varioLine$gamma)
title('Semi-variogram for TPAH GW Conc. without NC029 (NCG + NYCDEP)')
legend(x = 0, y = 2, "Model Variogram. Dir: East, tol:25deg", col = "black", lty = 1, bty = "n")


# Krige onto spatialGrid
#kr_sg <- krige(TPAH_ug_L ~ 1, locations = data, newdata = sg, model=null.vgm)
kr_sg <- gstat(id="VSeepKR", formula = TPAH_ug_L~1, data=dat,
...
plot(us_str_points_spdfSUB$TPAH_ug_L, extract(kr_sgRaster, us_str_points_spdfSUB),
     ylab = "OK Pct Fines (%)", xlab = "Measured Pct Fines (%)",
     pch = 20, cex = 1.5, ylim = c(0, 100), xlim = c(0, 100))
lines(c(-1000, 1000), c(-1000, 1000), lty = 2, lwd = 1)
...
for(i in 1:nrow(dat@data)){
  ...
  vgm = variogram((TPAH_ug_L) ~ 1, data = tmp,
  ...
  # Inverse distance weighting
  idw_sg0 <- krige0(formula = TPAH_ug_L~1, data = tmp,  newdata = data[i, ],
...
plot(data@data$TPAH_ug_L, (LOOCV_Pred0),
     ylab = "Pct Fines LOOCV OK prediction (%)", xlab = "Measured Pct Fines (%)",
     pch = 20, cex = 1.5, xlim = c(0, 100), ylim = c(0, 100))
lines(c(-1000, 1000), c(-1000, 1000), lty = 2, lwd = 1)
...
# g <- gstat(id="VSeepIDW", formula = TPAH_ug_L~1, data=data,
...
# null.vgm <- vgm(var(data$TPAH_ug_L), "Sph", sqrt(areaSpatialGrid(sg))/4,
