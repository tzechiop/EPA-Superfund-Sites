...
fp <- file.path("S:", "Projects", "NewtownCreek", "Data", "GW")
...
# Hydro Grid
fp <- file.path("U:", "Newtown_Creek_Files", "GIS", "NC04")
...
#reproj_grd$Xgrid <- reproj_grd$II - 13
#reproj_grd$Ygrid <- reproj_grd$JJ - 33
nc_grd_over$Xgrid <- nc_grd_over$II - 13 #X
nc_grd_over$Ygrid <- nc_grd_over$JJ - 33 #Y
...
sg_sgdf$II <- (sg_sgdf$s1 - 0.5) + 13
sg_sgdf$JJ <- (sg_sgdf$s2 - 0.5) + 33
...
# sub_out <- us_str_points_spdfSUB@data[c("SampleLoc", "X", "Y", "AvgQ_cmDay", "ModVqCMD")]
# fp <- file.path("U:", "Newtown_Creek_Files", "Data", "Time_Series_Data")
...
fp <- file.path("U:", "Newtown_Creek_Files", "GIS")
